...
**A native desktop GUI for compiling Bitcoin Core and Electrs from source**
...
│  Build Directory: ~/Downloads/bitcoin_builds             [Browse…]   │
├─────────────────────────────────────────────────────────────────────┤
...
│                    [ 🚀  Start Compilation ]                         │
├─────────────────────────────────────────────────────────────────────┤
│  System: macOS 14.5 arm64  |  Homebrew: /opt/homebrew  |  CPUs: 8   │
└─────────────────────────────────────────────────────────────────────┘
```
...
|---|---|---|
| macOS Apple Silicon | `aarch64-apple-darwin` | `BitForge-macos-arm64.zip` |
...
| Bitcoin Core | CMake | Wallet, GUI, tests, and optional network extras disabled |
...
# Debian/Ubuntu
sudo apt-get update
sudo apt-get install -y build-essential pkg-config cmake git clang libclang-dev \
  libevent-dev libx11-dev libxcb1-dev libxcursor-dev libxrandr-dev libxi-dev \
  libxkbcommon-dev libwayland-dev libudev-dev libdbus-1-dev

# Fedora
sudo dnf install -y gcc gcc-c++ pkgconf-pkg-config cmake git clang clang-devel \
  libevent-devel libX11-devel libxcb-devel libXcursor-devel libXrandr-devel \
  libXi-devel libxkbcommon-devel wayland-devel systemd-devel dbus-devel

# Arch
sudo pacman -S --needed base-devel pkgconf cmake git clang libevent libx11 \
  libxcb libxcursor libxrandr libxi libxkbcommon wayland systemd dbus
```
...
The macOS bundle script supports Apple Silicon only.
...
│                  · compile_bitcoin(): clone/update → cmake → copy
...
│                  · check_rust_installation(): probe → optional Homebrew install on macOS
...
│                  · find_brew(): Apple Silicon Homebrew path check
│                  · setup_build_environment(): platform-aware PATH dedup
│                  · LIBCLANG_PATH plus DYLD_LIBRARY_PATH/LD_LIBRARY_PATH where needed
│
...
tokio multi-thread runtime (worker threads = min(CPU count, 8))
   ...
   ├─ deps::check_dependencies() → tokio::process native package/tool probes
